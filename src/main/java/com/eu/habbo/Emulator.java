...
import com.eu.habbo.habbohotel.commands.EventScheduler;
import com.eu.habbo.habbohotel.events.AutoEventManager;
...
public final class Emulator {
    ...
    // Sobrescreve o System.out para aplicar a cor verde a todas as saídas
    ...
    // Define a versão do emulador usando um molde de formatação para facilitar a leitura.
// O %s funciona como um "espaço em branco" que será preenchido pelas variáveis na ordem em que aparecem.
    public static final String version = String.format("Rio Emulador - 2026 %s.%s.%s %s", MAJOR, MINOR, BUILD, PREVIEW);
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            if (PREVIEW.toLowerCase().contains("beta")) {
                System.out.println("Warning, this is a beta build...");
                promptEnterKey();
            }

            // ==========================================
            // ARTE ASCII DE INICIALIZAÇÃO
            // ==========================================
            System.out.println("");
            System.out.println(GREEN + "__________.__        ");
            System.out.println(GREEN + "\\______   \\__| ____  ");
            System.out.println(GREEN + " |       _/  |/  _ \\ ");
            System.out.println(GREEN + " |    |   \\  (  <_> )");
            System.out.println(GREEN + " |____|_  /__|\\____/ ");
            System.out.println(GREEN + "        \\/           ");
            System.out.println(""); // Pula uma linha no final para ficar organizado

            LOGGER.info("Rio Emulador 2026 - versão: " + version);
            LOGGER.info("Projeto baseado no Arcturus.");
    ...
    private static void dispose() {
        ...
        LOGGER.info("Stopping Aura Emulador {}", version);

        try {
            if (Emulator.getPluginManager() != null)
                Emulator.getPluginManager().fireEvent(new EmulatorStartShutdownEvent());
        } catch (Exception e) { LOGGER.error("Erro ao disparar evento de desligamento", e); }

        try {
            if (Emulator.cameraClient != null)
                Emulator.cameraClient.disconnect();
        } catch (Exception e) { LOGGER.error("Erro ao desligar a Camera", e); }

        try {
            if (Emulator.rconServer != null)
                Emulator.rconServer.stop();
        } catch (Exception e) { LOGGER.error("Erro ao parar o RCON Server", e); }

        try {
            if (Emulator.gameEnvironment != null)
                Emulator.gameEnvironment.dispose();
        } catch (Exception e) { LOGGER.error("Erro ao limpar GameEnvironment", e); }

        try {
            if (Emulator.getPluginManager() != null)
                Emulator.getPluginManager().fireEvent(new EmulatorStoppedEvent());
        } catch (Exception e) { LOGGER.error("Erro ao disparar evento de parada de plugin", e); }

        try {
            if (Emulator.pluginManager != null)
                Emulator.pluginManager.dispose();
        } catch (Exception e) { LOGGER.error("Erro ao limpar PluginManager", e); }

        try {
            if (Emulator.config != null) {
                Emulator.config.saveToDatabase();
            }
        } catch (Exception e) { LOGGER.error("Erro ao salvar configurações na Database", e); }

        try {
            if (Emulator.gameServer != null)
                Emulator.gameServer.stop();
        } catch (Exception e) { LOGGER.error("Erro ao parar o GameServer", e); }

        LOGGER.info("Stopped Aura Emulador {}", version);
        ...
        } catch (Exception e) { LOGGER.error("Erro ao desligar as Threads", e); }
    }
    ...
    public static Date modifyDate(Date date, String timeString) {
        // Converte o Date antigo para o formato moderno LocalDateTime
        LocalDateTime ldt = LocalDateTime.ofInstant(date.toInstant(), ZoneId.systemDefault());

        Matcher m = Pattern.compile("(([0-9]*) (second|minute|hour|day|week|month|year))").matcher(timeString);

        // Forma clássica e super segura de criar o Map (Sem o erro do '<>')
        Map<String, ChronoUnit> map = new HashMap<String, ChronoUnit>();
        ...
        while (m.find()) {
            try {
                int amount = Integer.parseInt(m.group(2));
                String what = m.group(3);
                ldt = ldt.plus(amount, map.get(what)); // O Java cuida da matemática
            } catch (Exception ignored) {}
        }
    ...
    private static String dateToUnixTimestamp(Date date) {
        // Reduzimos 6 linhas confusas para 1 linha exata e muito mais rápida.
        return String.valueOf(date.toInstant().getEpochSecond());
    }

    public static Date stringToDate(String date) {
        try {
            // Analisador de texto para Data ultra-rápido
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
            LocalDateTime ldt = LocalDateTime.parse(date, formatter);
            return Date.from(ldt.atZone(ZoneId.systemDefault()).toInstant());
        } catch (Exception e) {
            LOGGER.error("Erro ao ler data", e);
            return null;
        }
    }
