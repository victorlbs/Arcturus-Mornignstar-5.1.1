...
public class RoomEfeitoCommand extends Command {
    public RoomEfeitoCommand() {
        super("cmd_danceall", Emulator.getTexts().getValue("commands.keys.cmd_efeitoall").split(";"));
    }
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        if (params.length == 2) {
            ...
            for (Habbo habbo : gameClient.getHabbo().getHabboInfo().getCurrentRoom().getHabbos()) {
                habbo.getRoomUnit().setEffectId(danceId, 0); // Aqui, 0 é a duração do efeito (instantâneo)
