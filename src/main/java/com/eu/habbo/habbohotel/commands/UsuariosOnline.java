...
public class UsuariosOnline extends Command {
    public UsuariosOnline() {
        super("cmd_staffonline", Emulator.getTexts().getValue("commands.keys.cmd_usuariosonline").split(";"));
    }
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        synchronized (Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos()) {
            ...
            for (Map.Entry<Integer, Habbo> set : Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos().entrySet()) {
                if (set.getValue().getHabboInfo().getRank().getId() >= 1) {
            ...
            StringBuilder message = new StringBuilder(Emulator.getTexts().getValue("Todos os online no Hotel"));
            message.append("\r\n");
